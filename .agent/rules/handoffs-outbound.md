...
another repo per `repo-layout.md`, left in place here until a
Totebox Session in project-source started in the destination repo
commits the add-side.

Pattern: see `~/Foundry/CLAUDE.md` §9 (cross-repo handoff addendum
pending — surfaced for Command Session in `cleanup-log.md` 2026-04-23
entry).

Maintained by a Totebox Session in this repo. Read by Command Session
during workspace review, and by a Totebox Session in project-source
started in the destination repo when it comes time to commit the
add-side. An outbox entry is passive state: it describes intent, but
nothing moves until Command Session, a Totebox Session in
project-source, or a Totebox Session picks it up.

Each entry carries a **Prescriptive actions** subsection — exact
commands the destination Totebox Session can execute mechanically
without needing context from this session. A destination Totebox
Session starting cold in the target repo can read this file, find
...
Totebox Session in this repo grep-detects that pattern in the
...
- **Opened:** 2026-04-23 by a Totebox Session
- **Notes:** Plain move — no rename, no content change.

### Prescriptive actions for a Totebox Session in content-wiki-documentation
...
5. When complete, the source-side `git rm` will be committed in a
   future Totebox Session in project-source. That session
...
- **Notes:** Rename on transit — destination filename drops the
  ...
  a Totebox Session in the destination repo should flag the content as
...
- **In-transit edit 2026-04-23:** line 902 updated to reflect the
  ...
  Totebox Session will receive the corrected path; no separate
...
6. When complete, the source-side `git rm` will be committed in
   a future Totebox Session in project-source. That
...
`.claude/rules/` rule file). The destination Totebox Session transforms
...
- **Source files (this repo):**
  ...
  - `app-console-bim/RESEARCH.md` (5,311 B) — joint orchestration+
    console research, copy 1 (intentional duplicate — Totebox Session —
...
- **Destination repo:** `content-wiki-documentation`
- **Destination paths:** TBD by destination Totebox Session per its own
...
3. Commit each topic file via the staging-tier helper. Use the
   ...
   is the grep pattern a future Totebox Session in project-source
   will detect to close this outbox entry.
   Example:
   ```
