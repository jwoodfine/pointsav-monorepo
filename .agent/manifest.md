...
tetrad:                          # upgraded from triad: 2026-04-28 per claim #37 / doctrine v0.0.10
  vendor:
    - repo: media-knowledge-documentation
      path: clones/project-editorial/media-knowledge-documentation/   # NOT local to this archive — see note below
      upstream: vendor/content-wiki-documentation   # bare vendor repo kept its pre-2026-05 name; local clone renamed
      focus: TOPIC content; ADRs; service summaries; glossary
      note: >
        Repoint 2026-07-09 — this leg's local clone was relocated to project-editorial on
        2026-06-09 (content-wiki-* -> media-knowledge-* GitHub rename + clone consolidation,
        see PROJECT-CLONES.md). TOPIC authorship for this cluster now happens in
        project-editorial's Totebox session, not here. This entry stays in the tetrad for
        cross-reference accuracy; it is not a clone directory inside this archive.
  ...
  wiki:                          # NEW Tetrad leg per convention §4
    - repo: vendor/content-wiki-documentation   # bare upstream repo name (pre-rename); local TOPIC authorship happens in project-editorial's media-knowledge-documentation clone, not here (repointed 2026-07-09)
      drafts_via: clones/project-knowledge/.agent/drafts-outbound/
      gateway: project-editorial Totebox session (was: project-language Task, retired terminology)
      planned_topics:
...
clones:
  # content-wiki-documentation row removed 2026-07-09 — was never actually present in this
  # archive (confirmed via `ls`); the wiki-content clone lives in project-editorial as
  # media-knowledge-documentation/ since the 2026-06-09 rename/relocation. See tetrad.vendor
  # and tetrad.wiki notes above for the corrected cross-reference.
  - repo: pointsav-monorepo
    role: sibling
    path: pointsav-monorepo/
    upstream: vendor/pointsav-monorepo
