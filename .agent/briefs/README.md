...
| [BRIEF-os-mediakit-product-family.md](BRIEF-os-mediakit-product-family.md) | active | os-mediakit (base) + app-mediakit-knowledge (first product, live) architecture + **build-out plan** (added 2026-08-10, operator-directed, Phase 3 hold lifted): real seL4/Microkit+vendor-libvmm appliance, structured like project-totebox's os-totebox/os-orchestration (researched directly — scripts, Cargo.tomls, code). Phases F (fix the found-broken plain-QCOW2 Format B) → 0 (preflight: BUILD_DIR isolation, aarch64 cross-compile) → G1→G4/SIGTERM → G-TLS (new ground — nginx+certbot in-guest, no precedent) → Deploy. Doctrine corrections + VM-topology sign-off already sent to Command, awaiting reply |
