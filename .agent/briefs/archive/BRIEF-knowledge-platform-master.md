...
updated: 2026-07-13
superseded_by: project-knowledge-ng-rewrite
verdict_source: .agent/drafts-outbound/archived/BRIEF-REVIEW-old-brief-verdict.md
---

> **Frontmatter fix (2026-07-13):** `supersedes: archive/BRIEF-knowledge-platform-master.md`
> was removed — no such file ever existed (confirmed via `git log --follow`); this brief was
> never itself a supersession of an archived copy. Replaced with the accurate relationship:
> `superseded_by: project-knowledge-ng-rewrite`. `verdict_source` path corrected to its real
> location under `drafts-outbound/archived/`. See `BRIEF-knowledge-ng-rewrite.md`'s
> "Constitutional invariants" and "Federation/MCP disposition" sections for what from this
> brief was carried forward into the rewrite and what was deliberately not.
